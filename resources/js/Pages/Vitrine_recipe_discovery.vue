...
const filteredRecipes = computed(() => {
    ...
    return rawRecipes.filter((recipe) => {
        ...
        if (searchQuery.value) {
            ...
            const typeMatch =
                (recipe.categories && recipe.categories.some(cat => cat.nom && cat.nom.toLowerCase().includes(query))) ||
                (recipe.recette_type?.nom && recipe.recette_type.nom.toLowerCase().includes(query));
        ...
        if (activeFilters.value.length > 0) {
            ...
            for (const filter of activeFilters.value) {
                ...
                if (filter === "Dessert") {
                    const hasDessert = (recipe.categories && recipe.categories.some(cat => cat.nom && cat.nom.toLowerCase().includes("dessert"))) || type.includes("dessert");
