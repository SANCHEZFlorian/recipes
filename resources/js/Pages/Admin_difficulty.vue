<template>
    <AdminLayout>
        <div class="py-6">
            <div class="max-w-[1520px] mx-auto px-4 sm:px-6 lg:px-8">
                <!-- Page Header -->
                <div class="mb-8 flex justify-between items-center">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">Niveaux de Difficulté</h1>
                        <p class="mt-1 text-sm text-gray-500">Gérez les différents niveaux de difficulté des recettes.</p>
                    </div>
                    <button @click="openCreateModal" class="premium-button-primary">
                        <i class="fas fa-plus mr-2"></i> Nouveau Niveau
                    </button>
                </div>

                <!-- Content Table -->
                <div class="bg-white shadow-sm rounded-lg overflow-hidden">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-20">ID</th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Difficulté</th>
                                <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="diff in difficulties" :key="diff.id">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-500">
                                    #{{ diff.id }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="h-10 w-10 flex-shrink-0 flex items-center justify-center bg-emerald-100 rounded-full text-emerald-600">
                                            <i class="fi fi-rr-flame text-lg"></i>
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900">
                                                {{ diff.nom }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <button @click="openEditModal(diff)" class="text-emerald-600 hover:text-emerald-950 mr-4 cursor-pointer">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button @click="confirmDelete(diff)" class="text-red-600 hover:text-red-900 cursor-pointer">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                </td>
                            </tr>
                            <tr v-if="difficulties.length === 0">
                                <td colspan="3" class="px-6 py-12 text-center text-gray-500">
                                    Aucun niveau trouvé.
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>

        <!-- Modal -->
        <div v-if="showModal" class="premium-modal-backdrop" @click.self="showModal = false">
            <div class="premium-modal-content max-w-md p-6 bg-white rounded-3xl shadow-2xl relative">
                <button @click="showModal = false" class="absolute top-4 right-4 text-slate-400 hover:text-slate-600 transition-colors">
                    <i class="fas fa-times text-lg"></i>
                </button>
                
                <h3 class="text-2xl font-black text-slate-900 mb-6">
                    {{ isEditing ? 'Modifier la difficulté' : 'Nouvelle difficulté' }}
                </h3>
                
                <div class="space-y-5">
                    <div>
                        <label class="premium-label">Nom</label>
                        <input v-model="form.nom" type="text" class="premium-input" placeholder="Ex: Très difficile" />
                    </div>
                </div>

                <div class="flex gap-3 mt-8">
                    <button @click="showModal = false" class="premium-button-secondary flex-1">
                        Annuler
                    </button>
                    <button @click="submitForm" class="premium-button-primary flex-1">
                        {{ isEditing ? 'Mettre à jour' : 'Ajouter' }}
                    </button>
                </div>
            </div>
        </div>
    </AdminLayout>
</template>

<script setup>
import { ref, reactive } from "vue";
import { router } from "@inertiajs/vue3";
import AdminLayout from "@/Layouts/AdminLayout.vue";

const props = defineProps({
    difficulties: Array
});

const showModal = ref(false);
const isEditing = ref(false);
const editingId = ref(null);

const form = reactive({
    nom: ""
});

const openCreateModal = () => {
    isEditing.value = false;
    form.nom = "";
    showModal.value = true;
};

const openEditModal = (diff) => {
    isEditing.value = true;
    editingId.value = diff.id;
    form.nom = diff.nom;
    showModal.value = true;
};

const submitForm = () => {
    if (isEditing.value) {
        router.put(route('admin.difficulties.update', editingId.value), form, {
            onSuccess: () => { showModal.value = false; }
        });
    } else {
        router.post(route('admin.difficulties.store'), form, {
            onSuccess: () => { showModal.value = false; }
        });
    }
};

const confirmDelete = (diff) => {
    if (confirm(`Voulez-vous vraiment supprimer le niveau "${diff.nom}" ?`)) {
        router.delete(route('admin.difficulties.destroy', diff.id));
    }
};
</script>
