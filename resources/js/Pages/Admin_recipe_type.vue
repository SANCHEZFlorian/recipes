                ...
                <div class="mb-8 flex justify-between items-center">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">
                            Types de Cuisson/Plats
                        </h1>
                        <p class="mt-1 text-sm text-gray-500">
                            Gérez les types de recettes (ex: Entrée, Plat, Dessert).
                        </p>
                    </div>
                    <button
                        @click="openAddModal"
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            >
                                ...
                                >
                                    {{ type.recette_categorie ? type.recette_categorie.nom : 'N/A' }}
                                </td>
                                <td
                        ...
                        <form @submit.prevent="submitForm">
                            <div class="px-6 py-4 space-y-4">
                                ...
                                <div>
                                    <label
                                        class="block text-sm font-medium text-gray-700 mb-1"
                                        >Catégorie Parent</label
                                    >
                                    <select
                                        v-model="form.recette_categorie_id"
                                        required
                                        class="w-full border-gray-200 rounded-xl shadow-sm focus:border-emerald-500 focus:ring-emerald-500"
                                    >
                                    ...
                                    <p
                                        v-if="form.errors.recette_categorie_id"
                                        class="mt-1 text-sm text-red-600"
                                    >
                                        {{ form.errors.recette_categorie_id }}
                                    </p>
                                </div>
                            </div>
...
const openEditModal = (type) => {
    ...
    form.recette_categorie_id = type.recette_categorie_id;
    form.clearErrors();
    isModalOpen.value = true;
};
...
const submitForm = () => {
    if (isEditing.value) {
        form.put(route("admin.recipe-types.update", editingId.value), {
            preserveScroll: true,
            onSuccess: () => closeModal(),
        });
    } else {
        form.post(route("admin.recipe-types.store"), {
            preserveScroll: true,
            onSuccess: () => closeModal(),
        });
    }
...
const deleteType = (id) => {
    if (confirm("Êtes-vous sûr de vouloir supprimer ce type de recette ?")) {
        router.delete(route("admin.recipe-types.destroy", id), {
            preserveScroll: true,
        });
    }
};
